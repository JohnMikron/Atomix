"""
Atomix — Regression Test Suite for Core Primitives and Layout
=============================================================
"""
...
class TestCoreTypeIgnores(unittest.TestCase):
    """No floating # type: ignore comments outside function bodies."""
    ...
    def test_no_floating_type_ignore_in_core(self):
        ...
        for i, line in enumerate(lines, 1):
            ...
            if (
                stripped.startswith("# ")
                and stripped.endswith("# type: ignore")
                and "==" not in stripped
            ):
                floating_lines.append(i)
...
class TestCoreCleanupExceptionHandling(unittest.TestCase):
    """_cleanup() should use except Exception, not bare except."""
    ...
    def test_no_bare_except_in_cleanup(self):
        ...
        self.assertEqual(
            len(matches),
            0,
            "Found bare except: in _cleanup(). Should be 'except Exception:'",
        )


class TestAtomSwapDoubleRead(unittest.TestCase):
    """Atom.swap() should not have a redundant write-lock pre-read."""
...
class TestNestedTransactionDepth(unittest.TestCase):
    """Nested transaction() should not trigger premature commit."""
