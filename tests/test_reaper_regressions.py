"""
Atomix — Regression Test Suite for Reaper and Watcher Audits
============================================================
"""
...
class TestReaperAtexit(unittest.TestCase):
    ...
    def test_init_source_has_atexit(self):
        ...
        self.assertIn("_reaper.stop", source, "atexit should register reaper.stop()")
    ...
    def test_reset_source_has_atexit(self):
        ...
        self.assertIn(
            "atexit.register", source, "reset() should register new reaper with atexit"
        )


class TestSeqLockMaxSpins(unittest.TestCase):
    ...
    def test_max_spins_value(self):
        ...
        self.assertIn("100000", source, "max_spins should be 100000, not 10000")
        # Should NOT contain the old 10000 value as the only spin limit
        self.assertNotIn(
            "max_spins: int = 10000\n", source, "max_spins should NOT be 10000 anymore"
        )


class TestAtomSwapMaxRetries(unittest.TestCase):
    ...
    def test_swap_source_raises_commit_exception(self):
        ...
        self.assertIn(
            "CommitException", source, "Atom.swap should raise CommitException"
        )
        self.assertIn("1000", source, "Max retries should be 1000")
...
class TestNotifyWatchersAudit(unittest.TestCase):
    ...
    def test_ref_notify_watchers_logs(self):
        ...
        self.assertNotIn(
            "pass", source.split("except")[1] if "except" in source else ""
        )
...
class TestMonitoringImport(unittest.TestCase):
    ...
    def test_monitoring_imports_correctly(self):
        ...
        monitoring_dir = os.path.join(
            os.path.dirname(os.path.dirname(__file__)), "examples", "ecosystem"
        )
        if os.path.exists(os.path.join(monitoring_dir, "monitoring.py")):
            ...
            try:
                ...
                spec = importlib.util.spec_from_file_location(
                    "monitoring", os.path.join(monitoring_dir, "monitoring.py")
                )
