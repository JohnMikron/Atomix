...
import threading
import time
from atomix_stm import Ref, Atom, dosync, atomically, PersistentVector, STMQueue
...
class TestAtomixSTM(unittest.TestCase):
    ...
    def test_public_api_imports(self):
        """Verify that 'ref' and 'atom' factory functions can be imported from top level."""
        from atomix_stm import ref, atom

        # Test ref factory function
        r = ref(10)
        self.assertIsInstance(r, Ref)
        self.assertEqual(r.deref(), 10)

        # Test atom factory function
        a = atom(20)
    ...
    def test_conflict_resolution(self):
        ...
        threads = [
            threading.Thread(target=t1_thread),
            threading.Thread(target=t2_thread),
        ]
