...
│   ├── api.py            # Public context manager, transaction, dosync API
│   ├── coordinator.py    # Transaction coordinator and reaper
│   ├── core.py           # Legacy core compatibility layer
│   ├── exceptions.py     # Custom exceptions
│   ├── locks.py          # Lock-free and read/write synchronization locks
│   ├── persistent.py     # Functional persistent data structures
│   ├── primitives.py     # Agent, Queue, and thread-local variables
│   ├── ref.py            # Transactional references (Ref, Atom)
│   ├── transaction.py    # Transaction logging and commit logic
│   └── versioning.py     # MVCC version stamping and transaction state
├── tests/
